...
> **State as of 2026-05-26 — v2.0.0 SHIPPED + five patch releases + v2.1 fully closed + v2.2 architecture-gap fix + v2.3 pre-Phase-1 finance backlog round.** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith** across eight phases. v2.0.0 (the modular monolith + Projects module) tagged 2026-05-24. Five content / fix patches followed in 24 hours, then v2.1 closed (n8n delivery layer + heartbeats), then v2.2 surfaced and closed one architecture gap from Checkpoint G, then v2.3 closed four of the six remaining pre-Phase-1 Finance backlog items:
>
...
- **Active branch:** `master`. **v2.3 is the latest tag** (pre-Phase-1 Finance backlog round: alignment + auto-calc + planner charges + mobile scroll). All seven tags pushed to origin: `v2.0.0` `v2.0.1` `v2.0.2` `v2.0.3` `v2.0.4` `v2.0.5` `v2.3`. Live site: https://metalbee66.github.io/financial-planner/.
- **Latest verified state on the live site (2026-05-26):** v2.3 pushed to master; GitHub Pages rebuild kicked off automatically. 174/174 E2E green pre-push. **Manual smoke recommended on the four flows landed in v2.3:** (1) Budget CY — verify the four money tables' columns line up across cards; (2) Budget CY — toggle the "Auto-calculate from contribution split" checkbox on Brad Regular, confirm the row becomes editable then re-locks; edit any outgoing amount and confirm Brad Regular / Diana Regular's weekly resyncs to `Math.ceil((totalOut − rent) / 2)`; (3) Planner — open an item, add a charge with a non-zero amount, verify Actual switches to the sum + "1 charge" badge + read-only span, then add a second and verify Actual = sum; delete both, verify Actual snaps back to expected and the input returns; (4) view on a phone-width browser (DevTools 360px) and confirm the planner week-table scrolls horizontally rather than getting clipped.
...
- **Next task:** v2.3 shipped 2026-05-26 (commits `a4be072` → `952ccb7`, tagged `v2.3`). Sensible candidates for the next session, in priority order:
  - **Add v2.3 E2E coverage** — no automated tests were added in v2.3. Worth a `v2.3 — Finance backlog round` describe block in `tests-e2e/smoke.spec.js` covering: data-money column-width assertion across all four budget tables; auto-calc input read-only + detail toggle + recompute-on-outgoings-change; charge add → Actual derives + badge + read-only span → delete → Actual snaps back; planner mobile scroll wrap at 360px. Pattern: per-phase describe with shared `createBudget()` / `openPlanner()` helpers similar to the existing per-phase blocks.
  - **Backlog #3** — CSV parsers for HSBC / ANZ / Westpac / Bankwest. Each is mechanical work but needs a real CSV sample per bank from Brad; ask before starting. NAB only today (see `parseNabCsv` in `js/modules/finance/import.js` for the shape to copy).
  - **Backlog #4** — Bank API. Out of scope for a code-only session; needs vendor pick (Plaid AU / Basiq / Yodlee), auth model decision, security review. Treat as a discovery session before any coding.
  - Or whatever surfaces next from Brad.
