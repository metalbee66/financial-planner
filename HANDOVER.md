...
> **State as of 2026-05-04 (evening):** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, and **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones) of v2.0.0 are complete and committed to **local** master. The Playwright E2E harness runs **51 tests** including a `tests.html` driver that executes the in-browser unit suite (**97 cases**) — single `npm run test:e2e` covers both layers in ~3.5 min. Real-Firebase + two-user smoke still unverified. **4 commits sit ahead of `origin/master` and need pushing** (see "Where to pick up" below). Phases 4–8 build out the rest of the Projects module — see [tasks/plan.md](../tasks/plan.md).
...
- **Milestone UI**: a "Mark as milestone" checkbox in the task panel below the description. Milestone task rows pick up a `task-row-milestone` class (soft accent-coloured highlight) and a leading ◆ glyph in the task name. The per-project task header shows a "◆ Milestones only" filter checkbox; checking it hides non-milestones via `applyTaskFilters` (preserves non-milestone parents of milestone subtasks). Filter state lives on `mode.taskFilters` and resets when navigating to a different project.
...
- **Active branch:** `master`. **Local master is 4 commits ahead of `origin/master`** — Tasks 3.2, 3.3, 3.4, 3.5 are all locally committed but not yet pushed. Run `git push` to deploy via GitHub Pages once you're ready.
- **Last commit:** `7c47901` — Milestone flag on tasks (Task 3.5)
- **Phase 3 commits on master (newest first; `*` = local-only, not on origin yet):**
...
- **Next task:** Phase 4, Task 4.1 — List view (sort, group, filter) ([plan §4.1](../tasks/plan.md#task-41-list-view-with-filtering--grouping)). Promote the existing per-project task list into a richer view with column sort (name / due date / priority), group-by toggle (status / assignee / none), and composable filters (assignee, status, milestones-only). Note: the milestones-only filter from Task 3.5 is already wired on `mode.taskFilters` — extend that shape rather than adding a parallel mechanism. M-sized.
- **Branch convention:** L/M-sized tasks **were** landed on `family-planner/<slug>` feature branches up to and including Task 3.1, but Tasks 3.2–3.5 went direct to master. The convention has effectively shifted to "everything direct to master, scoped per-task commits, full suite passes before push." Stick with that unless a Phase-4-style L task (timeline, 4.2) makes a feature branch worthwhile for safer rollback.

### Manual smoke on the deployed site (Checkpoints C + D)

> **Push first.** The 4 Phase-3.2–3.5 commits are local-only. `git push` to master, then wait ~30 sec for GitHub Pages to rebuild, then run the manual checks below.

The Playwright suite (`npm run test:e2e`, 51 tests, ~3.5 min) covers the **functional** acceptance criteria for Phase 1.1, 1.2, 2.1, 2.2, and 3.1–3.5 plus a Phase 0 module-shell regression check, plus a `tests.html` driver that runs all 97 data-layer unit cases. Firebase is blocked at the network layer in tests so they run hermetically against localStorage.
...
9. **Console check.** Zero red errors on either browser after the above. (Browser-extension `asynchronous response… message channel closed` chatter is not an app issue — ignore it.)

If any of those regress, the rollback is `git revert <hash>` per task — newest first: `7c47901` (3.5), `618310d` (3.4), `67cf64f` (3.3), `7ea9693` (3.2). The four sit cleanly on top of `fdaacf7` so reverting any one in isolation is safe (the activity feed in 3.3 references the dep wrappers; reverting 3.3 alone would re-orphan the comments section under the old "Comments" name, so revert 3.2+3.3 together if going back that far). Earlier-phase rollbacks unchanged.

### Branch cleanup

Tasks 3.2–3.5 each landed direct on local master without feature branches, so there's nothing new to clean up locally. State unchanged from the prior handover:
...
- Origin has `family-planner/phase-2-tasks` lingering — schedule a cleanup routine if one isn't already pending. Check via `git ls-remote --heads origin 'family-planner/*'`.
...
1. **`npm run test:e2e`** (~3.5 min, chromium-only, runs against the local dev server which auto-starts if not already up). 51 tests in `tests-e2e/smoke.spec.js`:
   - Phase 1.1 / 1.2 / 2.1 / 2.2 / 3.1 / **3.2 / 3.3 / 3.4 / 3.5** acceptance criteria (UI through-and-through).
   - Phase 0 module-shell regression check.
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the 97 pure-data tests in `js/modules/projects/data.test.js` (project + task + subtask + dependency + comment + event + attachment + milestone helpers: create / validate / list mutators / sanitisers / `completedAt` transitions / `findSubtasks` / `promoteSubtasksInList` / `deleteTaskCascadeFromList` / `addDependency` / `removeDependency` / `wouldCreateCycle` / `countBlockingDeps` / `addCommentToTask` / `addEventToTask` / `taskPatchEvents` / `addAttachmentToTask` / `removeAttachmentFromTask` / `taskAttachmentSize` / `formatBytes` / `isMilestone` defaults).
...
**Known flakes.** Two intermittents, both rooted in Python's single-threaded `http.server` choking under fast-fire load:
- `page.goto: timeout` / `net::ERR_ABORTED` in `beforeEach` on a random test in the suite (~1-in-3 full runs). Re-run the failing test in isolation with `npx playwright test --grep "<the failing test's text>"` to confirm flake vs. regression. Always reproduces as passing when isolated.
- The 600 KB file-attachment test in Phase 3.4 amplifies the load and increases the flake rate slightly.
- Mitigation if it gets annoying: add `retries: 1` for non-CI runs in `playwright.config.js`. Held off because it papers over the dev-server issue rather than fixing the root cause.
