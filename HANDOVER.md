...
**Where the scrapers live.** `e:/Projects/Family Planner/scrapers/` is a top-level sibling of `app/`, **outside the public `app/` git repo by design** (scrapers contain login-flow logic for real netbanks; should not be public). For v2.4 in-flight, scrapers are untracked locally — version-control strategy decision deferred (private GitHub repo vs git submodule vs untracked-forever) until pilot ships. See [tasks/user-actions.md → v2.4 wrap](tasks/user-actions.md) for the open decision.

**Bracket of what's done at this handover (2026-05-28):**
- T1 ✅ — `scrapers/` bootstrap (package.json, `playwright.config.js`, `hello.spec.js`, `README.md`, `.gitignore`); `npm install` + `npx playwright install chromium` (~112 MB) ran cleanly; smoke test passed (10.7s).
- T8 ✅ — `DEFAULT_BANK_INBOX` + `sanitiseBankInbox` + `isValidBalanceRecord` + `BALANCE_CATEGORIES` in `data.js`; `parseHsbcCsv` + `parseCsvLine` + `parseHsbcDate` + `parseHsbcAmount` in `import.js`; 23 unit cases in new `app/js/data.test.js` (registered in `tests.html` alongside the projects suite); full `npm run test:fast` 2/2 green.
- Phase 0 user-actions UA1 done (KeePass = KeePassXC); UA2–UA5 pending Brad.
- T2/T2.5/T2.6 (interactive scraper writing) blocked on UA2–UA5; T9–T11 (browser-side build-out) are pure-code, unblocked, queued for next session.
