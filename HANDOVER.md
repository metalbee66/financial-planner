...
> **State as of 2026-05-07:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), and **Phase 4.1 + 4.2** (list view + Gantt timeline view) of v2.0.0 are complete and committed. Phase 3 manual smoke + push-to-prod completed earlier today; Phase 4.1 was pushed at the start of this session. The Playwright E2E harness runs **72 tests** including a `tests.html` driver that executes the in-browser unit suite (**131 cases**) — single `npm run test:e2e` covers both layers in ~5–6 min. Real-Firebase + two-user smoke for 4.1 / 4.2 still unverified. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake — full suite ran clean end-to-end. Phase 4.3 (calendar) + Phases 5–8 build out the rest of the Projects module — see [tasks/plan.md](../tasks/plan.md).
...
- **Run `npm run test:e2e` before committing any UI change.** 72 tests, ~5–6 min. The suite includes a `tests.html` driver that runs the in-browser data-layer unit suite (131 cases) — both layers verified in one command. If anything fails, fix the code or update the test — do not commit red. Use `--grep "<phase>"` (e.g. `npx playwright test --grep "Phase 4.2"`) for fast subset runs during dev — full suite once at the end.
...
- **Active branch:** `master`. Phase-3 + Phase-4.1 commits + handover are pushed to `origin/master` (4.1 deployed at the start of this session). The Phase-4.2 commit lands fresh on top.
- **Last commit:** `5c90cfd` — Timeline (Gantt) view (Task 4.2)
- **Phase 4 commits on master (newest first; `*` = local-only, not on origin yet):**
  - `5c90cfd`* — Timeline (Gantt) view (Task 4.2): bars, milestone diamonds, dependency arrows
...
- **Next task:** Phase 4, Task 4.3 — Calendar view (month grid) ([plan §4.3](../tasks/plan.md#task-43-calendar-view-month-grid)). M-sized. With both List and Timeline live, the calendar slot is the third view in the existing tab strip — extend `DETAIL_VIEW_OPTIONS` in `js/modules/projects/index.js` and add a `renderCalendarBody` companion to `renderListBody` / `renderTimelineBody`.
- **Branch convention:** L/M-sized tasks **were** landed on `family-planner/<slug>` feature branches up to and including Task 3.1, but Tasks 3.2–3.5, 4.1, and 4.2 all went direct to master with scoped per-task commits and a green test suite before push. This is now the established convention: direct to master with full suite green.

### Manual smoke on the deployed site (Checkpoint E — Phase 4.1 + 4.2)

> **Push first.** The Phase-4.2 commit is local-only. `git push` to master, then wait ~30 sec for GitHub Pages to rebuild, then run the manual checks below.

The Playwright suite (`npm run test:e2e`, 72 tests, ~5–6 min) covers the **functional** acceptance criteria for Phase 1.1, 1.2, 2.1, 2.2, 3.1–3.5, **4.1**, and **4.2** plus a Phase 0 module-shell regression check, plus a `tests.html` driver that runs all 131 data-layer unit cases. Firebase is blocked at the network layer in tests so they run hermetically against localStorage.
...
11. **Console check.** Zero red errors on either browser after the above. (Browser-extension `asynchronous response… message channel closed` chatter is not an app issue — ignore it.)

If Phase 4.2 regresses, rollback is `git revert <Phase-4.2-hash>` — single self-contained commit on top of `686bc0b`. Earlier-phase rollbacks unchanged: 4.1 → `f558f41`, 3.5 → `7c47901`, 3.4 → `618310d`, 3.3 → `67cf64f`, 3.2 → `7ea9693`.

### Branch cleanup

Phase 4.2 landed direct on local master, no feature branch. Nothing to clean up locally.
...
1. **`npm run test:e2e`** (~5–6 min, chromium-only, runs against the local dev server which auto-starts if not already up). 72 tests in `tests-e2e/smoke.spec.js`:
   - Phase 1.1 / 1.2 / 2.1 / 2.2 / 3.1 / 3.2 / 3.3 / 3.4 / 3.5 / **4.1** / **4.2** acceptance criteria (UI through-and-through).
   - Phase 0 module-shell regression check.
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the 131 pure-data tests in `js/modules/projects/data.test.js` (all the Phase-1–3.5 helpers plus 4.1's `sortTasks` / `filterTasks` / `groupTopLevelTasks` plus 4.2's `computeTimelineRange` / `computeTaskBars`).
