...
> **State as of 2026-05-19:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), **all of Phase 4** (4.1 list view + 4.2 Gantt timeline + 4.3 month-grid calendar), **all of Phase 5** (5.1 Overview + 5.2 My Tasks + 5.3 Dashboard + 5.4 Files), and **Task 6.1** (audit-event → notification trigger map) of v2.0.0 are complete, committed, and pushed. **2026-05-19: 5 of 9 polish-backlog items resolved** (PB.1 autofocus, PB.2 dropdown contrast, PB.3 reset-filters button, PB.5 dashboard chart headroom, PB.6 popup→redirect sign-in). **PB.4 (timeline dep arrows) was attempted then shelved** — three rounds of fixes (y-offset, SVG width, visibility boost) still produced straight diagonals cutting through intervening rows; arrows removed entirely from the Timeline view. Remaining open backlog: PB.7 (project-status auto-promote — needs spec), PB.8 (dashboard drill-down), PB.9 (joint-assignee — needs spec). **Checkpoint F manual smoke passed 2026-05-15** (14 clean / 2 pass-with-caveat / 0 fail) — see [tasks/todo.md → Polish backlog](tasks/todo.md) for the 9 polish items surfaced (Timeline dep arrows, Dashboard chart scaling, dropdown contrast, joint-assignee feature, project-status auto-promote, etc.). The Playwright E2E harness runs **112 tests** including a `tests.html` driver that executes the in-browser unit suite (**251 cases** after 6.1) — single `npm run test:e2e` covers both layers in ~9 min. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). Phases 6 (rest) – 8 build out the remaining Projects-module work — see [tasks/plan.md](tasks/plan.md).
