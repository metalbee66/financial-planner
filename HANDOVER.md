...
> **State as of 2026-05-24:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. **All of Phase 6** (6.1 trigger map + 6.2 in-app bell & prefs + 6.3 instant email queue + 6.4 daily digest accumulation + 6.5 admin panel) and **Task 7.1** (celebrations) are now complete on the browser side, committed locally on `master`. **Two n8n workflow builds are deferred** to [tasks/user-actions.md → Surfaced during implementation](tasks/user-actions.md) until the Pre-Phase-6 infra is up (n8n container on SEi14 Geekom, M365 Outlook credential, Firebase service-account Bearer): the instant email-queue drainer (every 60s) and the daily-8am digest sender. Without these the browser-side queue + digest_pending bucket grow in Firebase but never deliver — full workflow shapes (Schedule → GET → SplitInBatches → Outlook Send → PATCH) are documented in user-actions.md. **Checkpoint G (two-user end-to-end notification flow)** therefore blocks on that infra. Prior state (still true): Phases 0–5 complete + polish round 9-of-9 done (PB.4 timeline dep arrows shelved); Checkpoint F manual smoke passed 2026-05-15; **manual two-tab Firebase smoke still owed** from the polish-round close-out. The Playwright E2E harness now runs **149 tests** including a `tests.html` driver that executes the in-browser unit suite (**~317 cases**) — single `npm run test:e2e` covers both layers in ~12 min. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). **Local commits unpushed** as of 2026-05-24 — see [Where to pick up](#where-to-pick-up). Next: Task 7.2 (local AI helpers) is the only remaining browser-side work before Phase 8 retirement of PM DLBooks.
...
- **Active branch:** `master`. **Phase 6 (6.1–6.5) browser-side complete + Task 7.1 (celebrations) complete**, all committed locally as of 2026-05-24. **5 commits ahead of `origin/master`** — `d2e56af` (6.2 bell+prefs) → `6a46680` (6.3 instant queue) → `e3563c4` (6.4 digest) → `445f5cf` (6.5 admin) → `905e0ef` (7.1 celebrations + Beelink→Geekom doc fix). Push when ready; deploy is the usual `git push` → GitHub Pages auto-rebuild.
- **n8n side of Phase 6 still deferred.** Two workflow builds are queued in [tasks/user-actions.md → Surfaced during implementation](tasks/user-actions.md): the 60-second instant email-queue drainer (6.3) and the daily-08:00 digest sender (6.4). Both block on the Pre-Phase-6 manual ops (none of which are ticked yet): n8n container on SEi14 Geekom, M365 Outlook credential, Firebase service-account Bearer, RTDB rules. Workflow node graphs + the concurrent-write caveat are recorded in user-actions.md so an agent (or you) can author them via the n8n MCP tools once the Tailscale IP + creds are available.
- **Checkpoint G (two-user end-to-end notification flow + daily digest) therefore blocks on the same infra.** The Playwright suite verifies everything the browser owns — enqueue, mark-read, admin retry, prefs persistence, digest accumulation, celebrations — but cannot prove the actual email lands. Plan a manual smoke once the workflows are live.
- **Earlier outstanding items:** Polish round 9-of-9 done (commits up to `8b25366`); PB.4 timeline dep arrows shelved; **manual two-tab Firebase smoke owed from the polish-round close-out** (deferred during the round; see [tasks/todo.md → Polish Checkpoint DONE](tasks/todo.md)).
...
- **Next task:** Task 7.2 — local AI helpers ([plan §7.2](tasks/plan.md#task-72-local-ai-helpers-no-api-calls)). M-sized. Five heuristic-only features (no external API): (1) task-name autocomplete from history, (2) auto-suggest due date from project span, (3) daily digest text on the Dashboard, (4) stale-project badge for projects with no updates in >14 days, (5) smart-sort by urgency score. All compute in browser; no network calls in DevTools while exercising them. Acceptance: each heuristic produces plausible output on demo data. After 7.2: **Phase 8** — migrate PM DLBooks → Projects (8.1 idempotent data migration), retire the legacy module + tab (8.2), walk through user-actions.md with the user, tag v2.0.0 (8.3).
- **Outside the build pipeline:** the two n8n workflows queued in user-actions.md can be authored at any point once the Geekom + M365 + Firebase Bearer creds are in place. They're orthogonal to the remaining browser-side work.
- **Branch convention:** L/M-sized tasks have gone direct to master with scoped per-task commits and a green test suite before push since Task 3.2. This is the established convention for the remainder of the project. The Tasks 6.2/6.3/6.4/6.5/7.1 commits also followed it — local-only at the moment, push when ready.
