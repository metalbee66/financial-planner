...
> **State as of 2026-05-07:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), and **all of Phase 4** (4.1 list view + 4.2 Gantt timeline + 4.3 month-grid calendar) of v2.0.0 are complete and committed. Phase 3 manual smoke + push-to-prod completed earlier today; Phase 4.1 + 4.2 were pushed during this session. The Playwright E2E harness runs **81 tests** including a `tests.html` driver that executes the in-browser unit suite (**144 cases**) — single `npm run test:e2e` covers both layers in ~6 min. Real-Firebase + two-user smoke for 4.1 / 4.2 / 4.3 still unverified. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). Phases 5–8 build out the rest of the Projects module — see [tasks/plan.md](../tasks/plan.md).
...
- **Next task:** Phase 5, Task 5.1 — Overview tab (all projects at a glance) ([plan §5.1](../tasks/plan.md#task-51-overview-tab-all-projects-at-a-glance)). M-sized. Cards for each project: name, % complete (done tasks / total), date range, participants, status, next milestone date, overdue task count.
- **Branch convention:** L/M-sized tasks **were** landed on `family-planner/<slug>` feature branches up to and including Task 3.1, but Tasks 3.2–3.5, 4.1, 4.2, and 4.3 all went direct to master with scoped per-task commits and a green test suite before push. This is now the established convention: direct to master with full suite green.
...
1. **`npm run test:e2e`** (~6 min, chromium-only, runs against the local dev server which auto-starts if not already up). 81 tests in `tests-e2e/smoke.spec.js`:
   - Phase 1.1 / 1.2 / 2.1 / 2.2 / 3.1 / 3.2 / 3.3 / 3.4 / 3.5 / **4.1** / **4.2** / **4.3** acceptance criteria (UI through-and-through).
   - Phase 0 module-shell regression check.
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the 144 pure-data tests in `js/modules/projects/data.test.js` (all the Phase-1–3.5 helpers plus 4.1's `sortTasks` / `filterTasks` / `groupTopLevelTasks` plus 4.2's `computeTimelineRange` / `computeTaskBars` plus 4.3's `getMonthGridCells` / `bucketCalendarTasks`).
