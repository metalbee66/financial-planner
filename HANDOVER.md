...
> **State as of 2026-05-21:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), **all of Phase 4** (4.1 list view + 4.2 Gantt timeline + 4.3 month-grid calendar), **all of Phase 5** (5.1 Overview + 5.2 My Tasks + 5.3 Dashboard + 5.4 Files), **Task 6.1** (audit-event → notification trigger map), and **the entire polish round (9 of 9 PB items)** are complete, committed, and pushed. **Polish round 2026-05-19 → 2026-05-21** specced and built three remaining backlog items: **PB.7** hybrid project status (derived default + `statusOverride` flag, single read path via `effectiveProjectStatus()`); **PB.9** joint-assignee via `task.assignees: string[]` (legacy `task.assignee` tolerated forever on read through `readAssignees()`, canonical Brad+Diana pair collapses to "Joint" chip); **PB.8** dashboard cards drill into an inline cross-project task list driven by a new `dashboardView` filter primitive. **PB.4 (timeline dep arrows) remains shelved** from the earlier round. **Checkpoint F manual smoke passed 2026-05-15** (14 clean / 2 pass-with-caveat / 0 fail) and the 2026-05-19 round of half the polish items closed in `94cbe78` / `1f28614` / `cb17652`. Final 9-of-9 close out: **manual two-tab Firebase smoke still owed** (deferred during the polish round; see [tasks/todo.md → Polish Checkpoint DONE](tasks/todo.md)). The Playwright E2E harness now runs **120 tests** including a `tests.html` driver that executes the in-browser unit suite (**307 cases** after the polish round) — single `npm run test:e2e` covers both layers in ~9 min. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). Phases 6 (rest) – 8 build out the remaining Projects-module work — see [tasks/plan.md](tasks/plan.md).
...
│       ├── projects/
│       │   ├── index.js                Module entry: list / detail / form views, task panel, subtask + dep + attachment + milestone wiring, activity feed, notification trigger wiring, all event handlers
│       │   ├── data.js                 Project + task schemas, validation, list mutators, sanitisers, subtask + dep + comment + event + attachment + milestone helpers, load/save, email↔participant resolver
│       │   ├── notifications.js        Audit-event → notification trigger map (Task 6.1): pure mapper + recipient resolver + dep-unblock derivation + time-based scan + per-user bucket helpers
│       │   └── data.test.js            Unit tests for the data layer (run via /tests.html, 251 tests)
│       └── pm-legacy/
...
- **Active branch:** `master`. Phase 5 is feature-complete + **Task 6.1 (notification trigger map) is feature-complete and tested**. **Checkpoint F manual smoke passed 2026-05-15** — verdict captured in [tasks/todo.md](tasks/todo.md) under Phase 5 with the polish backlog (PB.1–PB.9) immediately below. PB.5 caveat fixed; PB.1, PB.2, PB.3, PB.6 also resolved. **PB.4 (timeline dep arrows) shelved** after three rounds of fixes — arrows removed entirely from Timeline. Remaining open: PB.7, PB.8, PB.9 (all need spec work).
...
- **Polish backlog commits on master (newest first):**
  - _next_ — Remove timeline dep arrows (PB.4 shelved)
  - `7fe169e` — Polish: timeline arrows visibility boost (PB.4 attempt, since reverted)
  - `70ca556` — Polish: timeline arrows SVG-width fix (PB.4 attempt, since reverted)
  - `16b4f10` — Polish: timeline dep arrows arrowheads + y-offset fix (PB.4 attempt, since reverted)
...
- **Next task:** Task 6.1 done (notifications produced + persisted; nothing consumes them yet). Proceed to Task 6.2 — in-app bell + per-user preferences UI ([plan §6.2](tasks/plan.md#task-62-in-app-bell--per-user-preferences-ui)). M-sized. Adds a bell icon in the shell header (visible across all modules) with unread count, click-to-open dropdown showing last 30 notifications for the current user, mark-as-read on click, preferences modal at `/household/family/projects/prefs/{user}`. The notifications already get written by 6.1 to `state.projectsData.notifications.<participantId>` so 6.2 is purely a UI + prefs layer over data that's already flowing. After 6.2: 6.3 wires the email queue + n8n workflow (needs external infra — see [user-actions.md → Pre-Phase-6](tasks/user-actions.md#pre-phase-6-n8n--m365-infrastructure-for-email)).
...
1. **`npm run test:e2e`** (~9 min, chromium-only, runs against the local dev server which auto-starts if not already up). 112 tests in `tests-e2e/smoke.spec.js`:
   ...
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the **251** pure-data tests in `js/modules/projects/data.test.js` (all the Phase-1–3.5 helpers plus 4.1's `sortTasks` / `filterTasks` / `groupTopLevelTasks`, 4.2's `computeTimelineRange` / `computeTaskBars`, 4.3's `getMonthGridCells` / `bucketCalendarTasks`, 5.1's `computeProjectProgress` / `countOverdueTasks` / `findNextMilestone` / `sortProjectsForOverview`, 5.2's `bucketTasksForUser` / `defaultMyTasksUser` / `collectMyTasksUserOptions`, 5.3's `computeDashboardMetrics` / `computeWeeklyCompletionBars`, 5.4's `collectAttachmentsByProject`, and **6.1's `eventToNotification` / `candidateRecipientsForEvent` / `deriveDependencyUnblockedTriggers` / `computeTimeBasedTriggers` / `addNotificationToBucket` / `processTrigger` + `emailToParticipantId`**).
