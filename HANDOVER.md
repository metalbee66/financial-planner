...
> **State as of 2026-05-08:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), **all of Phase 4** (4.1 list view + 4.2 Gantt timeline + 4.3 month-grid calendar), and **the first half of Phase 5** (5.1 Overview tab + 5.2 My Tasks per-user summary) of v2.0.0 are complete and committed. 5.1 was pushed end-of-day yesterday; 5.2 was pushed during this session. The Playwright E2E harness now runs **95 tests** including a `tests.html` driver that executes the in-browser unit suite (**168 cases**) — single `npm run test:e2e` covers both layers in ~7 min. Real-Firebase + two-user smoke for 4.1 / 4.2 / 4.3 / 5.1 / 5.2 still unverified. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). Phase 5 still owes 5.3 Dashboard + 5.4 Files; Phases 6–8 build out the rest of the Projects module — see [tasks/plan.md](../tasks/plan.md).
...
│       │   └── data.test.js            Unit tests for the data layer (run via /tests.html, 168 tests)
│       └── pm-legacy/
...
- **Active branch:** `master`. All Phase 5.1 + 5.2 commits are on `origin/master` — 5.2 was pushed end of session, so the live site at https://metalbee66.github.io/financial-planner/ has the Projects sub-tabs (Overview | My Tasks) and the four bucketed sections. Manual smoke for 4.1 / 4.2 / 4.3 / 5.1 / 5.2 still pending.
- **Last commit:** `0c9f15e` — My Tasks tab: cross-project per-user summary view (Task 5.2). Worktree clean. `master == origin/master`.
...
- **Next task:** Phase 5, Task 5.3 — Dashboard tab ([plan §5.3](../tasks/plan.md#task-53-dashboard-tab)). M-sized. Six metric cards (active projects, open tasks, overdue, due this week, completion rate last 30 days, upcoming milestones next 14 days) plus an inline-SVG mini bar chart of completion rate per week (last 8 weeks). Auto-refreshes via the existing Firebase listener — no chart library, vanilla JS / SVG only. After 5.3 lands, 5.4 (Files summary by project, S-sized) closes Phase 5.
- **Branch convention:** L/M-sized tasks **were** landed on `family-planner/<slug>` feature branches up to and including Task 3.1, but Tasks 3.2–3.5, 4.1–4.3, 5.1, and 5.2 all went direct to master with scoped per-task commits and a green test suite before push. This is now the established convention: direct to master with full suite green.

### Manual smoke on the deployed site (Checkpoint E — Phase 4 complete; Phase 5 in flight)

> **Already deployed.** Phase 5.2 was pushed at the end of this session. Hard-refresh https://metalbee66.github.io/financial-planner/ (Ctrl+Shift+R) and run the manual checks below. The harness covered the functional acceptance criteria automatically — these checks cover what the harness can't (real Firebase, two-user, visual layout).

The Playwright suite (`npm run test:e2e`, 95 tests, ~7 min) covers the **functional** acceptance criteria for Phase 1.1, 1.2, 2.1, 2.2, 3.1–3.5, 4.1, 4.2, 4.3, **5.1**, and **5.2** plus a Phase 0 module-shell regression check, plus a `tests.html` driver that runs all 168 data-layer unit cases. Firebase is blocked at the network layer in tests so they run hermetically against localStorage.
...
1. **`npm run test:e2e`** (~7 min, chromium-only, runs against the local dev server which auto-starts if not already up). 95 tests in `tests-e2e/smoke.spec.js`:
   - Phase 1.1 / 1.2 / 2.1 / 2.2 / 3.1 / 3.2 / 3.3 / 3.4 / 3.5 / 4.1 / 4.2 / 4.3 / **5.1** / **5.2** acceptance criteria (UI through-and-through).
   - Phase 0 module-shell regression check.
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the 168 pure-data tests in `js/modules/projects/data.test.js` (all the Phase-1–3.5 helpers plus 4.1's `sortTasks` / `filterTasks` / `groupTopLevelTasks`, 4.2's `computeTimelineRange` / `computeTaskBars`, 4.3's `getMonthGridCells` / `bucketCalendarTasks`, 5.1's `computeProjectProgress` / `countOverdueTasks` / `findNextMilestone` / `sortProjectsForOverview`, and 5.2's `bucketTasksForUser` / `defaultMyTasksUser` / `collectMyTasksUserOptions`).
