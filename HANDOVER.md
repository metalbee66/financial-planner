...
> **State as of 2026-07-02 — v2.0.0 SHIPPED + five patch releases + v2.1 fully closed + v2.2 architecture-gap fix + v2.3 pre-Phase-1 finance backlog round + v2.4 bank-scraping pipeline LIVE + TAGGED.** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith** across eight phases. v2.0.0 (the modular monolith + Projects module) tagged 2026-05-24. Five content / fix patches followed in 24 hours, then v2.1 closed (n8n delivery layer + heartbeats), then v2.2 surfaced and closed one architecture gap from Checkpoint G, then v2.3 closed four of the six remaining pre-Phase-1 Finance backlog items, then v2.4 delivered the bank-data scraping pipeline end-to-end and was tagged 2026-07-02 (after the API path was abandoned twice — see entry below):
>
...
> - **v2.3** — Pre-Phase-1 Finance backlog round (4 of 6 remaining items closed in one session, 4 scoped commits + 1 docs commit). **#1 Section alignment** (`a4be072`): money tables (Income / Outgoings / Split / Residual × CY + NY) now share fixed column widths via `table-layout: fixed` + a `data-money` attribute; mobile drops back to auto-layout. **#6 Contribution auto-calc** (`2304cb6`): `saveBudgetCY` / `saveBudgetNY` now run `recomputeAutoCalcContributions` which sets `weekly = Math.ceil((totalOut − rent) / 2)` on every contributionItem flagged `autoCalc: true`; rows render read-only with a detail-panel toggle to unlock. **#7 Mobile polish** (`1a589e2`): planner week-table wrapped in `.table-scroll` so the 8-col layout scrolls horizontally on phones instead of being clipped by `.section-card { overflow: hidden }`. **#2 Planner charges** (`952ccb7`): each weekActuals item / contribution entry can carry `charges: [{id, date, amount, payee, comment}]`; when present, `actual = sumCharges(charges)` (read-only span), a chevron in col-item reveals an expandable detail row with add/delete form, status auto-syncs against expected, YTD rolls up via the existing helper. Remaining backlog: #3 CSV parsers (needs Brad's bank CSV samples) and #4 Bank API (needs vendor + auth research).
> - **v2.4 (LIVE + tagged 2026-07-02)** — Bank-scraping pipeline shipped end-to-end: **HSBC + NAB scrapers live on the Geekom (daily), n8n ingest live (`fpHsbcIngest01`/`fpNabIngest01`, every 8 h), 243 real transactions flowing into Firebase `bank_inbox` → the Import tab (Brad-confirmed).** Deploy needed the `C:\BankScrapes` mount + `NODE_FUNCTION_ALLOW_BUILTIN=fs,path` on the shared n8n compose. Transfers + interest auto-classify into their own review tabs (kept out of the budget: transfers net to zero, interest is already in the mortgage payment). HSBC NBSP-separator normalisation in `txHash`/details. NAB breakthrough: shadow-DOM SPA cracked with ARIA-role locators (`scrapers/nab-a11y.mjs`) — reuse for Westpac/ANZ/Bankwest. The **4-week burn-in was de-gated from the tag** (ongoing ops, not a release gate — see CHANGELOG v2.4). Post-tag open: NAB2 (empty KeePass entry), Selfwealth + AMP ingest (authored, undeployed — scrapers unbuilt), scaling the remaining logins. Original in-progress notes retained below for the decision trail:
>   - Original pilot scope was HSBC + Selfwealth + AMP, run from the Geekom via Windows Task Scheduler, drains CSV/JSON into `C:\BankScrapes\<bank>\YYYY-MM-DD\` for n8n to ingest into a new Firebase `bank_inbox/{transactions,balances}/` key, surfaced in the Family Planner Import tab + accounts.js. **Backlog #4 pivoted twice on 2026-05-28** before landing here. Round 1: direct Basiq API integration — abandoned the same day after research confirmed Basiq's A$500/mo minimum floor + 12-mo contract makes household-scale spend ~$6,000/yr min, uneconomical. Round 2: PocketSmith as a consumer-priced Basiq reseller — Brad subscribed (~$120/yr), tried it, and discovered HSBC AU's CDR scope structurally excludes loan accounts — the load-bearing piece of his planner setup. PocketSmith cancelled within the trial window. Both superseded spec docs ([`tasks/SPEC-bank-api.md`](tasks/SPEC-bank-api.md), [`tasks/plan-bank-api.md`](tasks/plan-bank-api.md)) kept on disk with ⛔ SUPERSEDED banners for the decision trail. Round 3: browser-automated scraping — sees what the netbank UI sees, so HSBC loans come through. Plan at `C:\Users\brads\.claude\plans\i-want-to-discuss-dapper-trinket.md`. Three-scraper pilot validates the three architectural patterns in the full account scope (HSBC = multi-account transactions loop × 6, Selfwealth = balance-only × 2, AMP = hybrid transactions+balance × 1). 4-week burn-in before scaling to remaining 6 logins (NAB1, NAB2, Westpac, ANZ, Bankwest, IBKR). **Progress at handover (2026-06-05): HSBC scraper LIVE + scheduled.** Phase-0 UA1–UA5 all done; T1, T8, T2 (HSBC), T3 (`lib/keepass.js`), T5/T6 (wrapper + daily 06:00 Task Scheduler) complete. Scrapers now in private repo `metalbee66/family-planner-scrapers`, deployed to + running on the Geekom (`nanny-prod-01`) via SSH. HSBC logs in headed (Akamai blocks headless — see the v2.4 notes section + the bank-decisions memory), MFA-free via a key-file vault + trusted-device profile, exports 6 accounts to `C:\BankScrapes\hsbc\<date>\`. First true unattended run = 2026-06-06 (verify next session). Remaining: Selfwealth (2 balance) + AMP (1 hybrid) scrapers, T7 (n8n ingest × 3), T9 (`firebase-sync` realtime listener), T10/T10.5 (browser Bank-inbox UI + accounts.js auto-populate — pure-code, unblocked), T11 (E2E), T12 (wrap + tag v2.4). New architecture-relevant lesson from the abandonment trail: [L2 in `tasks/lessons.md`](tasks/lessons.md#l2--detailed-spec-written-before-vendor-was-hands-on-validated-2026-05-28) — validate vendors with a hands-on trial against Brad's specific accounts BEFORE writing a detailed spec/plan; the PocketSmith planning work was superseded the same day Brad tried it.
>
...
## v2.3 architecture notes — finance polish round (alignment + auto-calc + charges)

Worth carrying forward — three patterns added in the 2026-05-26 finance backlog close-out that any future Finance-module change should respect.

**`data-money` attribute on aligned tables (#1).** Four budget tables per year — Income / Outgoings / Split / Residual — share fixed column widths via `table-layout: fixed` + a `data-money` HTML attribute. Cols 1–5 line up across cards (col 1 = 220px, cols 2–5 = 110px each); Outgoings's extra cols 6–7 (Pay Cycle + First Payment) sit at the right of its table only. Tables use `width: max-content; max-width: 100%` so Income / Split / Residual end at col 5 with empty space on the right — alignment is preserved, narrower tables don't stretch out of register. Bonuses keeps auto layout (2 cols, different shape, not in the alignment goal). Mobile (<768px) reverts to `table-layout: auto; width: 100%` so the tables stay within the viewport — alignment is sacrificed for readability at that breakpoint. **If you add a new money-style budget table, mark it with `data-money` to inherit the alignment; if you add a non-money table, leave the attribute off.**

**Save-hook auto-calc (#6).** `saveBudgetCY` and `saveBudgetNY` in `data.js` now call `recomputeAutoCalcContributions(data)` before `fbSave`. That helper iterates `data.contributionItems`, and for every item with `autoCalc: true` sets `item.weekly = Math.ceil((totalOut − rent) / 2)` — exactly the same formula `renderSplit` uses to compute "Contribution Split (rounded)". Idempotent (recomputing twice produces the same answer). Brad Regular and Diana Regular were flagged `autoCalc: true` from v1; the recompute never fired until v2.3. **Initial load does NOT trigger a recompute** — the user sees whatever's persisted; the first edit syncs everything. This avoided silently mutating Firebase on first load after deploy. Auto-calc rows render as static `fmt(w)` (no input); the detail panel carries an "Auto-calculate from contribution split" checkbox that toggles `item.autoCalc` and unlocks manual editing. **If you ever need to bypass auto-calc programmatically, set `item.autoCalc = false` before calling save.**

**Planner charges (#2).** Each `weekActuals[w].items[name]` / `.contributions[name]` bucket entry can now carry `charges: [{id, date, amount, payee, comment}]`. `makeCharge({date, amount, payee, comment})` builds one with an `ch_<base36-time>_<4 random base36>` id. `sumCharges(charges)` is the canonical reader. When `charges.length > 0`:
- `actual` is derived (`sumCharges(charges)`) and the Actual input is replaced with a read-only `.actual-derived` span.
- A "N charges" `.charges-badge` renders next to the item name.
- Status auto-resolves: `Math.abs(sum − expected) < 0.01 ? 'confirmed' : 'adjusted'` — same rule the manual input flow uses.
- YTD Var rolls up via the existing `computeYtdVariance` which sums `saved.actual` — charges-driven rows feed through unchanged, no separate code path needed.

`recomputeFromCharges(entry, weekIdx, itemName, type)` in `planner.js` is the helper that syncs `actual` + `status` whenever charges change; it also snaps `actual` back to `expected` and clears status to `pending` when charges drops to zero (deleting the last charge resets the row). Both add and delete event handlers call it; the status-button click path skips the actual-clobber when `bucket[itemName].charges.length > 0` (so manually flipping "confirmed" doesn't overwrite the derived total).

**Data migration.** `migrateWeekActuals(weekActuals)` in `data.js` is called by `loadWeekActuals` (localStorage) AND by `firebase-sync.js` `initialSync` (`state.weekActuals = migrateWeekActuals(fbWA)`) to backfill `charges: []` on every existing entry. **There is no realtime listener for `week_actuals_cy26`** — never has been; peer edits still require a page reload. Pre-existing behaviour, not introduced or regressed by v2.3.

**UI state across renders.** `openChargeDetails` is a module-level `Set<"${type}|${itemName}">` in `planner.js` tracking which charge-detail rows are open. The chevron-click handler toggles set membership and visibility; `buildChargesDetailRow` reads the set when rendering so detail rows stay open across the renderWeek calls that happen after each charge add/delete. The set is cleared in `onWeekChange` so navigating weeks always lands on collapsed rows. Detail rows live as a sibling `<tr class="planner-detail-row">` immediately following the main row; `nextElementSibling` is used to find the detail row from a chevron click — no key map needed.

**Tests.** v2.3 ships with **no new automated tests** — 174 E2E + 372 unit cases unchanged. The new helpers (`sumCharges`, `makeCharge`, `migrateWeekActuals`, `computeAutoCalcContribution`, `recomputeAutoCalcContributions`) are pure and small enough to verify by inspection; the UI flows were manually smoked locally. **A follow-up to add E2E coverage for the four flows (alignment selectors, auto-calc input read-only + toggle, charges add/delete + Actual derivation, planner mobile scroll wrap) would be useful** before the next big Finance-module change. Pattern to follow: `tests-e2e/smoke.spec.js` already has Phase-0 module shell regression; a new `v2.3 — Finance backlog round` describe block would mirror the existing per-version describes.

---

## v2.4 architecture notes — bank-data scraping pilot (HSBC + Selfwealth + AMP)

Worth carrying forward — establishes the cross-cutting `bank_inbox` Firebase key + the per-bank scraper pattern that subsequent banks (NAB1/NAB2/Westpac/ANZ/Bankwest/IBKR) will copy.

**Two report types, two ingestion paths.** The `bank_inbox/` parent has two siblings:
- `bank_inbox/transactions/{txHash}` — one record per transaction. `txHash = <source>|<accountSlug>|<date>|<amount>|<details>` keeps same-day-same-amount rows across different accounts from colliding (HSBC has 6 accounts; without the slug in the hash, two coffee charges on the same day from different cards would dedup into one). The record shape matches what `parseNabCsv` already outputs (`{date, dateStr, amount, isRefund, account, source, txType, details, category, merchant, glLine, isDuplicate}`) so the existing `autoSuggest` + `applyToPlanner` flow consumes it unchanged.
- `bank_inbox/balances/{accountSlug}` — single record per account, overwritten on each balance scrape (`PUT` semantics, not `PATCH`). Shape: `{accountSlug, balance, asOf, institution, category}` where `category ∈ {'banking', 'super', 'investment'}`. `isValidBalanceRecord(obj)` in `data.js` gates writes (used by the n8n ingest workflow and the eventual browser render path). Surfaces in `accounts.js` alongside manually-entered balances with an "auto" tag + `asOf` timestamp; manual override flips a row to manual entry going forward.

**Scraper / ingest / browser split (per-bank failure isolation):** Each bank has its own scraper script (`scrapers/<slug>.spec.js`, outside the `app/` repo at `e:/Projects/Family Planner/scrapers/`), its own KeePass entry in `C:\Vault\senseai.kdbx` (`Family Planner - <slug> netbank`), its own persisted `storageState.json` at `C:\Vault\fp-state\<slug>.json` (carries the bank's trusted-device cookie so MFA only fires ~monthly), its own Windows Task Scheduler entry (daily 06:00 — actual launch jitters 0–90 min inside a PowerShell wrapper to avoid bot-signal), its own healthchecks.io check (1d × 2h grace), and its own n8n ingest workflow. A failure in one bank does NOT cascade — HSBC could break for weeks and AMP + Selfwealth continue running fine. The decision gate at the end of the 4-week burn-in is per-bank: ship the successful scrapers, fall back to manual CSV import for the fragile ones.

**Scraper output → n8n ingest → Firebase.** Scraper saves CSVs (transactions) and `.balance.json` files (balances) to `C:\BankScrapes\<bank>\YYYY-MM-DD\`. Three n8n workflows poll `C:\BankScrapes\<bank>\<today>\` every 15 min — `FamilyPlanner: bank inbox - hsbc ingest` (CSV → parse → PATCH `bank_inbox/transactions/`), `FamilyPlanner: bank inbox - selfwealth ingest` (JSON → validate → PUT `bank_inbox/balances/{slug}`), `FamilyPlanner: bank inbox - amp ingest` (hybrid; routes by file extension). Each ingest moves the processed file to `processed/YYYY-MM-DD/` so the watcher doesn't re-fire; trailing healthcheck (`FamilyPlanner-<Bank>IngestCron`).

**HSBC CSV parser quirks** (relevant when other-bank parsers are written): HSBC's format is 4 columns (`Transaction Date, Description, Amount, Balance`) with a trailing comma → empty 5th column. Leading whitespace on every value (header AND data). Date format is `D MMM YYYY` or `DD MMM YYYY` (e.g. `25 May 2026`, `01 May 2026`) — different from NAB's. Amounts are quoted strings with comma thousands separator (`"-1,181.24"`, `"4,522.00"`); the new `parseCsvLine` helper in `import.js` handles embedded commas in quoted fields, which `parseNabCsv` didn't need. No category column (unlike NAB). `parseHsbcCsv(text, accountSlug)` takes a 2nd `accountSlug` arg because HSBC's CSV doesn't include the account it came from — the scraper provides it via the filename, and the ingest workflow passes it to the parser. Loan-account rows look identical to transaction-account rows in CSV shape (just mostly-negative running balances).

**Bot-detection mitigations (baked in by design):** 1 run/day per bank (below most thresholds); random 0–90 min start jitter via PowerShell wrapper; random 30–180s inter-account pause inside each scraper; per-account loop order randomised; persistent browser fingerprint via storageState (rotating fingerprints scream bot; same fingerprint every visit looks human); no User-Agent spoofing (counterintuitively, UA changes are MORE bot-like than human); residential IP (Geekom is on Brad's home connection, not datacenter); same OS-installed Chromium version Brad uses manually. The plan's risk-+-rollback section spells out the escalation path if a bank's anti-bot tooling fires (`playwright-extra` stealth plugin port, or per-bank abandonment).

**Sample-data privacy guard (one-time, 2026-05-28).** Real bank exports get treated as private — `C:\Vault\fp-samples\` is the canonical location. `app/.gitignore` blocks `sample-data/*.{csv,pdf,qif,qfx}` from being committed, with `!sample-data/Transactions.csv` exempted because that file is already in public git history. Future bank-export samples MUST go into `C:\Vault\fp-samples\` (filename pattern: `<bank>-<description>-YYYY-MM-DD.<ext>`), never into the repo.

**Where the scrapers live.** `e:/Projects/Family Planner/scrapers/` is a top-level sibling of `app/`, **outside the public `app/` repo by design** (real netbank login flows; must stay private). **VC decision resolved 2026-06-05: its own private repo `metalbee66/family-planner-scrapers`.** Code is authored on the dev box and deployed to the **Geekom** (`nanny-prod-01`) at `C:\FamilyPlanner\scrapers\` via `scp` over SSH (`senseAi-admin@100.67.178.56`, key `~/.ssh/geekom_ed25519`). The scrapers RUN on the Geekom, not the dev box.

**HSBC scraper is LIVE + scheduled (2026-06-05).** Full operational detail + hard-won gotchas live in the memory [project_bank_api_decisions.md](file:///C:/Users/brads/.claude/projects/e--Projects-Family-Planner/memory/project_bank_api_decisions.md) — read it before touching the scrapers. Headlines:
- **Must run HEADED** — HSBC's Akamai returns "Something went wrong / reference: EAC" for headless, bundled Chromium, AND codegen's recorder. Uses real Chrome (`channel:'chrome'`) + anti-automation flags. Unattended headed runs via **Sysinternals Autologon** (senseai-admin) + a Task Scheduler task with **LogonType Interactive**, daily 06:00, wrapper `run-hsbc.ps1` (0–90min jitter + healthcheck ping).
- **Credentials** in a dedicated **key-file** KeePassXC vault `C:\Vault\familyplanner.kdbx` (unlocked by `C:\Vault\fp-state\familyplanner.keyx`, NTFS-locked) — separate from `senseai.kdbx`; read via `keepassxc-cli` in `lib/keepass.js`. Trusted-device session persisted in a Chrome user-data-dir (`C:\Vault\fp-state\hsbc-profile`) → MFA-free.
- **6 accounts** export to `C:\BankScrapes\hsbc\<date>\<slug>.csv`: everyday, carrum-downs (=Primary Home Loan), cranbourne, mentone, home-value, stock-assets. Download is an inline overlay (CSV default) whose button opens a blank popup that streams the file — caught at the CONTEXT level. Wrapper test 6/6 green; first true unattended run = 2026-06-06 (verify next session).

**What's done / what's next (v2.4):**
- ✅ T1 (`scrapers/` bootstrap), T8 (`parseHsbcCsv` + bank-inbox data layer in `data.js`, 23 unit cases), T2 (HSBC scraper), T3 (`lib/keepass.js`), T5/T6 (wrapper + scheduled task). Phase-0 UA1–UA5 all done.
- ⏳ **Next:** Selfwealth scraper (2 balance accounts), AMP scraper (1 hybrid tx+balance), then T7 (n8n ingest → Firebase `bank_inbox`), T9/T10/T10.5 (browser Bank-inbox UI + accounts.js balance auto-populate — pure-code, unblocked), T11 (E2E). The bank-inbox Firebase shape + parser already exist from T8.
...
3. ~~**CSV parsers**~~ — **superseded 2026-05-28 by v2.4 browser-scraping pilot**. HSBC's `parseHsbcCsv` shipped in v2.4 T8 (`e188213`) because the scraper output needs parsing anyway. The other banks (ANZ, Westpac, Bankwest) would each get a parser as part of their scraper rollout post-pilot — same `parseNabCsv`-shape pattern, gated on real CSV samples from Brad's manual exports going into `C:\Vault\fp-samples\` (NOT the repo — see the sample-data privacy guard in v2.4 architecture notes).
4. ~~**Bank API**~~ — **explored + abandoned 2026-05-28**. Two iterations: direct Basiq (A$500/mo floor) → PocketSmith reseller ($120/yr but HSBC loans not CDR-shareable). See v2.4 entry in the top-of-file status block + `tasks/lessons.md` L2 + memory `project_bank_api_decisions.md` for the full trail. Backlog replaced by the v2.4 browser-scraping pilot.
...
- **Active branch:** `master`. **v2.3 is the latest tag**; v2.4 is in-progress (no tag yet — won't tag until burn-in passes the per-bank decision gate). All seven tags pushed to origin: `v2.0.0` `v2.0.1` `v2.0.2` `v2.0.3` `v2.0.4` `v2.0.5` `v2.3`. Live site: https://metalbee66.github.io/financial-planner/.
- **Latest verified state on the live site (2026-05-28):** v2.4 T1 + T8 pushed to master (commits `672e447`, `c9b9176`, `e188213`). `npm run test:fast` 2/2 green (Phase 0 module-shell regression + in-browser data-layer suite — the latter now includes 23 new `bank inbox + HSBC parser` unit cases). UI surface is unchanged from v2.3 — the v2.4 browser-side bits (Bank inbox sub-section in Import tab, accounts.js auto-populate) are queued for the next session (T9 / T10 / T10.5).
...
- **v2.4 — Bank-scraping pilot commits on master (newest first, no tag yet):**
  - `e188213` — Finance: bank inbox data layer + HSBC CSV parser (v2.4 T8) — `DEFAULT_BANK_INBOX` + `sanitiseBankInbox` + `isValidBalanceRecord` in `data.js`; `parseHsbcCsv` + `parseCsvLine` + `parseHsbcDate` + `parseHsbcAmount` in `import.js`; 23 unit cases in new `app/js/data.test.js`; registered in `tests.html`.
  - `c9b9176` — Repo: gitignore `sample-data/*.{csv,pdf,qif,qfx}` except historical Transactions.csv. Privacy guard against accidental commit of real bank exports — real samples live in `C:\Vault\fp-samples\` instead.
  - `672e447` — Docs: pivot Backlog #4 to scraper pilot — supersede earlier specs. Both `tasks/SPEC-bank-api.md` (PocketSmith rewrite) + `tasks/plan-bank-api.md` kept on disk with ⛔ SUPERSEDED banners; `tasks/todo.md` v2.4 section replaced (was CSV parsers, now scrapers); `tasks/lessons.md` gains L2 (validate-vendor-before-detailed-spec lesson from the wasted PocketSmith planning work).
- **v2.3 — Pre-Phase-1 finance backlog round commits on master (newest first, tagged `v2.3`):**
  - `8e3fcf2` — Docs: HANDOVER mark finance backlog #1 / #2 / #6 / #7 closed
...
- **Next task:** v2.4 is the active workstream. Plan at `C:\Users\brads\.claude\plans\i-want-to-discuss-dapper-trinket.md`. Priority order for the next session:
  - **T9 (S)** — `app/js/firebase-sync.js`: add realtime listener on `bank_inbox` (single listener fires on either `transactions` or `balances` sibling change) + `initialSync` branch loading the key + default-push when Firebase is empty. Register a new `renderImportTab` render hook via `registerRenderHooks`. Initialise `state.bankInbox = { transactions: {}, balances: {} }` in `state.js`. Pure-code, no Brad-input gate.
  - **T10 (M)** — Browser Bank inbox sub-section in `app/js/modules/finance/index.js` + `import.js`: replace the existing "Bank API (Coming Soon)" `.section-card` with a real "Bank inbox" `.section-card`; new `renderBankInboxTab()` filters `state.bankInbox.transactions` to `applied === false` and feeds them through the existing `autoSuggest` + `renderImportTab` table; Apply-to-Planner extended to PATCH `applied: true` + `appliedAt` + `glLine` back into the inbox row per `markBankInboxApplied(ids, glLines)`. `shell.js` wires the `renderImportTab` hook to call both `renderMappings` AND `renderBankInboxTab`.
  - **T10.5 (S)** — `app/js/modules/finance/accounts.js`: extend the existing render to merge `state.bankInbox.balances` entries alongside manually-entered balances with an "auto" tag + relative-time `asOf`; stale-flag (>48h) gets a warning indicator; manual override flips an auto row to manual entry going forward.
  - **T11 (S)** — Two new E2E describe blocks in `tests-e2e/smoke.spec.js`: `bank-api — HSBC inbox` (3 scenarios: render seeded inbox rows, GL-assign + Apply writes back applied:true, re-seed same row doesn't re-show) AND `bank-api — auto-populated balances` (2 scenarios: balances render in accounts.js with the auto indicator; manual override behaviour).
  - **Then unblocked by Brad's UA2–UA5 (see [tasks/user-actions.md](tasks/user-actions.md)):** T2/T2.5/T2.6 interactive headed scraper-writing sessions for HSBC, Selfwealth, AMP. Plus T3 (`scrapers/lib/keepass.js` wrapping `keepassxc-cli` per UA1) + T4 (`scrapers/lib/healthcheck.js`) + T5 (three PowerShell wrappers) + T6 (three Task Scheduler entries) + T7 (three n8n ingest workflows).
- **Phase 0 user-actions Brad owns** (do in parallel with my code work):
  - UA1 ✅ KeePass = KeePassXC
  - UA2 — Create 3 KeePass entries
  - UA3 — Headed-browser trusted-device enrollment per bank
  - UA4 — Create 3 healthchecks.io checks
  - UA5 — Create folders
...
7. **For v2.4 (in progress):** the plan lives at `C:\Users\brads\.claude\plans\i-want-to-discuss-dapper-trinket.md` (outside the `app/` repo — it's a Claude-managed plan file). The scraper code lives at `e:/Projects/Family Planner/scrapers/` (sibling of `app/`, also outside the repo — see v2.4 architecture notes for the version-control deferral). Real bank-export samples live at `C:\Vault\fp-samples\` (NOT in the repo). Auto-memory at `C:\Users\brads\.claude\projects\e--Projects-Family-Planner\memory\` carries the cross-session decision trails — `MEMORY.md` is the index; `project_bank_api_decisions.md` is the load-bearing one for v2.4.
