...
> **State as of 2026-05-09:** v1.0.0 (Financial Planner) was renamed to **Family Planner** and restructured as a **modular monolith**. Phases 0, 1, 2, **all of Phase 3** (Tasks 3.1 deps + cycle detection, 3.2 comments thread, 3.3 activity/audit feed, 3.4 file attachments, 3.5 milestones), **all of Phase 4** (4.1 list view + 4.2 Gantt timeline + 4.3 month-grid calendar), and **most of Phase 5** (5.1 Overview tab + 5.2 My Tasks per-user summary + **5.3 Dashboard tab**) of v2.0.0 are complete and committed. 5.3 landed during this session. The Playwright E2E harness now runs **105 tests** including a `tests.html` driver that executes the in-browser unit suite (**182 cases**) — single `npm run test:e2e` covers both layers in ~8 min. Real-Firebase + two-user smoke for 4.1 / 4.2 / 4.3 / 5.1 / 5.2 / 5.3 still unverified. **`server.py` uses `ThreadingHTTPServer`** which eliminated the `ERR_CONNECTION_REFUSED` / `ERR_ABORTED` flake; **a `FAMILY_PLANNER_NO_BROWSER` env var** suppresses the auto-open browser tab during test runs (set automatically by `playwright.config.js`). Phase 5 still owes 5.4 Files; Phases 6–8 build out the rest of the Projects module — see [tasks/plan.md](../tasks/plan.md).
...
- **Next task:** Phase 5, Task 5.4 — Files summary by project ([plan §5.4](../tasks/plan.md#task-54-files-summary-by-project)). S-sized. Sub-tab listing all attachments grouped by project; columns project / task / file-or-URL name / type / size / added-by / added-on; click row → jump to task. Useful for "where did I attach the X doc?". After 5.4 lands, Phase 5 closes and Phase 6 (email notifications via n8n + in-app bell) begins. **Note**: 5.4 should add a fourth `Files` sub-tab beside Overview / My Tasks / Dashboard — or live as a per-project tab. Plan §5.4 reads as cross-project, so likely the former (extend `LIST_SUBTABS` array).
- **Branch convention:** L/M-sized tasks **were** landed on `family-planner/<slug>` feature branches up to and including Task 3.1, but Tasks 3.2–3.5, 4.1–4.3, 5.1, 5.2, and 5.3 all went direct to master with scoped per-task commits and a green test suite before push. This is now the established convention: direct to master with full suite green.
...
15. **Console check.** Zero red errors on either browser after the above. (Browser-extension `asynchronous response… message channel closed` chatter is not an app issue — ignore it.)

If Phase 5.3 regresses, rollback is `git revert b0564a2` — single self-contained commit. Earlier-phase rollbacks unchanged: 5.2 → `0c9f15e`, 5.1 → `1f3b3af`, 4.3 → `c0c0f47`, 4.2 → `5c90cfd`, 4.1 → `f558f41`, 3.5 → `7c47901`, 3.4 → `618310d`, 3.3 → `67cf64f`, 3.2 → `7ea9693`.
...
1. **`npm run test:e2e`** (~8 min, chromium-only, runs against the local dev server which auto-starts if not already up). 105 tests in `tests-e2e/smoke.spec.js`:
   - Phase 1.1 / 1.2 / 2.1 / 2.2 / 3.1 / 3.2 / 3.3 / 3.4 / 3.5 / 4.1 / 4.2 / 4.3 / **5.1** / **5.2** / **5.3** acceptance criteria (UI through-and-through).
   - Phase 0 module-shell regression check.
   - **In-browser unit-suite driver** — visits `/tests.html` and asserts the summary reports 0 failures. This is what runs the 182 pure-data tests in `js/modules/projects/data.test.js` (all the Phase-1–3.5 helpers plus 4.1's `sortTasks` / `filterTasks` / `groupTopLevelTasks`, 4.2's `computeTimelineRange` / `computeTaskBars`, 4.3's `getMonthGridCells` / `bucketCalendarTasks`, 5.1's `computeProjectProgress` / `countOverdueTasks` / `findNextMilestone` / `sortProjectsForOverview`, 5.2's `bucketTasksForUser` / `defaultMyTasksUser` / `collectMyTasksUserOptions`, and 5.3's `computeDashboardMetrics` / `computeWeeklyCompletionBars`).
