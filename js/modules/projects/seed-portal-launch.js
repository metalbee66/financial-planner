 ...
 *
 * Six phases: domain hardening → unbuilt tabs → backfill for existing
 * clients → practice readiness → pilot → staged launch.
 *
...
export function seedPortalLaunchProject() {
    ...
    // ── Phase 1 ───────────────────────────────────────────────────────────
    addPhase(
        'Phase 1 — Finish or cut the unbuilt portal tabs',
        'Invoices, Reports and Business Solutions are placeholder cards. The announcement names all three.',
        [
            {
                name: 'Decide which of the three unbuilt tabs ship for launch',
                description: 'Anything cut comes out of the announcement.',
                priority: 'high',
                milestone: true,
            },
            {
                name: 'Cut the Invoices tab from the launch scope',
                description: 'No CRM invoice model exists: no crm_invoices table, no portal_invoices migration, no publisher. The warehouse invoices table is the client\'s own Xero AR/AP.',
                priority: 'high',
            },
            {
            ...
            },
            {
                ...
                description: 'Current fixtures are synthetic; sign conventions are inverted and the P&L has no Gross Profit row. Then generate one pack by hand and read it before setting reports_enabled to 1.',
                priority: 'high',
            },
            {
                name: 'Decide what the Business Solutions tab contains',
                description: 'No spec, data model or publisher exists. Content decision, or remove the tab.',
            },
            {
