...
## Pre-Bank-API (v2.4 — browser-automated scraping pilot) — IN PROGRESS

> Replaces the abandoned PocketSmith pre-action list (which never reached the user-actions stage before the trial killed the value case). Plan at [`C:\Users\brads\.claude\plans\i-want-to-discuss-dapper-trinket.md`](file:///C:/Users/brads/.claude/plans/i-want-to-discuss-dapper-trinket.md). Scoped to 3 scrapers (HSBC + Selfwealth + AMP); blocks T2 / T2.5 / T2.6 interactive scraper-writing sessions.

- [x] **UA1** Confirm KeePass flavour — **KeePassXC** (2026-05-28). Scraper credential read uses `keepassxc-cli`.
- [ ] **UA2** Create 3 KeePass entries in `C:\Vault\senseai.kdbx`: `Family Planner - HSBC netbank`, `Family Planner - Selfwealth`, `Family Planner - AMP`. Each with the respective username + password. ~6 min.
- [ ] **UA3** From the Geekom's regular Chrome/Edge (NOT headless Playwright), log into HSBC, Selfwealth, and AMP manually one at a time → complete MFA → enroll "Remember this device" / equivalent where offered. Three trusted-device sessions established that the scrapers will inherit via `storageState`. ~15 min.
- [ ] **UA4** Create 3 healthchecks.io checks (1d period × 2h grace each) under `metalbee66@gmail.com`: `FamilyPlanner-HsbcScraperCron`, `FamilyPlanner-SelfwealthScraperCron`, `FamilyPlanner-AmpScraperCron`. Save URLs. ~6 min.
- [ ] **UA5** Create folders: `C:\BankScrapes\hsbc\`, `C:\BankScrapes\selfwealth\`, `C:\BankScrapes\amp\`, `C:\BankScrapes\logs\`, `C:\Vault\fp-state\`. ~1 min.
...
- [ ] **2026-05-28** — During T9/T10 work: **decide version-control strategy for `e:/Projects/Family Planner/scrapers/`** (currently untracked, sibling of `app/`). Options: (a) new private GitHub repo, (b) git submodule under `app/`, (c) untracked-forever (operational-only, like n8n workflows). Decide before the pilot ships.
