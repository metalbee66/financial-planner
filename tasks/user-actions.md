...
- [~] **UA-NAB.6** NAB1 Task Scheduler entry `FamilyPlanner-Nab1ScraperDaily` REGISTERED 2026-06-30 (Interactive, senseAi-admin, **daily 06:30** — offset 30 min from the 06:00 HSBC task; logged in routines.md). **Verify the first 06:30 run lands a CSV + the healthcheck greens.** NAB2 task pending its scraper. **Still owed: deploy `fpNabIngest01` to n8n** — blocked on the same `C:\BankScrapes` mount + `NODE_FUNCTION_ALLOW_BUILTIN` flag as HSBC (see `app/n8n/bank-ingest.BUILD-SHEET.md`).
