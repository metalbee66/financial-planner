...
- [ ] **v2.2.1** Wire a daily server-side overdue/due-soon scan in n8n — S — _**DEPLOYED inactive 2026-06-23** via SSH→`docker exec n8n` (no API key). Workflow `Family Planner: overdue/due-soon scan` (n8n id `fpOverdueScan01`), instant-only, 07:00 Melbourne. Logic in [`n8n/overdue-scan.code-node.js`](../n8n/overdue-scan.code-node.js) (self-check 24/24); deployed JSON [`n8n/overdue-scan.workflow.json`](../n8n/overdue-scan.workflow.json). Heartbeat uses the dual-filter sentinel topology (PROVEN: a `[]`-emitting node halts the chain + `alwaysOutputData` doesn't rescue zero-input — so the naive linear chain would skip the heartbeat on quiet days, the v2.1.1 bug). **Remaining (Brad, n8n UI — see [BUILD-SHEET](../n8n/overdue-scan.BUILD-SHEET.md)): (1) create `FamilyPlanner-OverdueScanCron` healthcheck + paste its ping URL over the Heartbeat placeholder; (2) one manual Execute to verify end-to-end (NOT yet run on the live server — CLI executor races port 5679); (3) Activate. Also delete the leftover `probeEmptyItems01` scaffolding workflow.** Digest-mode deferred (instant-only)._
  ...
  - **Heartbeat:** wire the standard trailing `Heartbeat` node with the empty-items-safe Merge → Set (Always Output Data, Execute Once) tail from v2.1.1 — most days the scan finds nothing, so this workflow is born with the empty-queue short-circuit risk. New healthchecks.io check `FamilyPlanner-OverdueScanCron` (period 1 day, grace 2 h).
  - **Verify:** seed a task with `dueDate` = yesterday + a known assignee whose prefs are instant-mode → run the workflow manually → an `email_queue` row appears and the InstantDrainer sends one email; run the workflow a **second** time same day → no duplicate row (dedupe holds). Repeat with a digest-mode assignee → entry lands in `digest_pending`, not `email_queue`.
- [ ] **v2.2.2** Update [routines.md](file:///C:/Users/brads/.claude/routines.md) Family Planner table + healthchecks table with the new `FamilyPlanner: overdue/due-soon scan` workflow + `FamilyPlanner-OverdueScanCron` check — XS — same turn as v2.2.1 landing.
