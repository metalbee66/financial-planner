...
- [ ] **v2.2.1** Wire a daily server-side overdue/due-soon scan in n8n — S — _**logic built + tested 2026-06-23**: [`n8n/overdue-scan.code-node.js`](../n8n/overdue-scan.code-node.js) (self-contained scan + prefs gate + instant/digest split, `node` self-check 24/24) and the step-by-step [`n8n/overdue-scan.BUILD-SHEET.md`](../n8n/overdue-scan.BUILD-SHEET.md). **Remaining = manual n8n-UI assembly + verify + activate** per the build sheet (only Brad can do this — it's in the n8n web UI on the Geekom)._
